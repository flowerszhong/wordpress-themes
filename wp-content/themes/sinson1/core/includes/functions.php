...
if (!function_exists('responsive_js')) {
	...
	function responsive_js() {
		...
		wp_enqueue_script('unslider', $template_directory_uri . '/core/' . $directory . '/unslider' . $suffix . '.js', array('jquery'), '2.0.7', true);
		wp_enqueue_script('sinson', $template_directory_uri . '/core/' . $directory . '/sinson' . $suffix . '.js', array('jquery', 'unslider'), '0.0.1', true);
...
function new_excerpt_more($more) {
	return ' <a class="read-more" href="' . get_permalink(get_the_ID()) . '">' . __('Read More', '查看更多') . '</a>';
}
