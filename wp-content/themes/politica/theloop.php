...
if (have_posts()):
	...
	while (have_posts()):the_post();?>
		...
		if (!is_single() && !is_page()) {?>
			<h2><a href="<?php the_permalink();?>" title="<?php printf($SMTheme->_('permalink'), the_title_attribute('echo=0'));?>" class='post_ttl'><?php the_title();
			?></a></h2>
			<?php } else {?>
		<h1><?php the_title();?></h1>
		<?php }
...
//Post meta (comments, date, categories)
	if (!is_page()) {?><p class='post-meta'>
		...
		if (comments_open(get_the_ID())) {
			?><img alt="" src="<?php echo get_template_directory_uri();?>/images/smt/comments.png">&nbsp;
			<span class="meta_comments"><?php comments_popup_link($SMTheme->_('noresponses'), $SMTheme->_('oneresponse'), $SMTheme->_('multiresponse'));
			?></span>
			<?php }
	...
	//Post content
	if (!is_single() && !is_page()) {
		if (!post_password_required()) {smtheme_excerpt('echo=1');} else {
			the_content('');
		}

		?><a href='<?php the_permalink();?>' class='readmore'><?php echo $SMTheme->_('readmore');
		?></a><?php
	} else {
		the_content('');
	}
